const { TodoList } = require('./todoList');

class Collection {
  constructor(todos) {
    this.todos = todos.slice();
  }
  get newTodoId() {
    const lastTodo = this.todos[this.todos.length - 1];
    const id = lastTodo && lastTodo.id + 1;
    return id || 1;
  }
  addNewTodo(title, tasks) {
    const id = this.newTodoId;
    const newTodo = new TodoList(id, title, []);
    tasks.map(task => newTodo.addTask(task));
    this.todos.unshift(newTodo);
  }
  deleteTodo(todoId) {
    const index = this.todos.findIndex(todoList => todoList.id == todoId);
    this.todos.splice(index, 1);
  }
  addTask(todoListId, taskName) {
    const index = this.todos.findIndex(todoList => todoList.id == todoListId);
    this.todos[index].addTask(taskName);
  }
  deleteTask(todoListId, taskId) {
    const index = this.todos.findIndex(todoList => todoList.id == todoListId);
    this.todos[index].deleteTask(taskId);
  }
  toggleTask(todoListId, taskId) {
    const index = this.todos.findIndex(todoList => todoList.id == todoListId);
    this.todos[index].toggleTask(taskId);
  }
  editTitle(todoListId, title) {
    const index = this.todos.findIndex(todoList => todoList.id == todoListId);
    this.todos[index].editTitle(title);
  }
  editTask(todoListId, taskId, name) {
    const index = this.todos.findIndex(todoList => todoList.id == todoListId);
    this.todos[index].editTask(taskId, name);
  }
  saveToFile(writer) {
    writer(JSON.stringify(this.todos));
  }
  static storeTodos(content) {
    const todos = content.map(TodoList.createNewTodoList);
    return new Collection(todos);
  }
}

module.exports = { Collection };
